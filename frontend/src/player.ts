...
export function mountPlayer(container: HTMLElement, opts: { playlist: string; poster?: string }): PlayerHandle {
  ...
  const qualityBtn = container.querySelector<HTMLButtonElement>(".pl-quality-btn")!
  const qualityMenu = container.querySelector<HTMLUListElement>(".pl-quality-menu")!
  ...
  if (Hls.isSupported()) {
    ...
    hls.on(Hls.Events.MANIFEST_PARSED, () => {
      levels = [{ level: -1, label: "Auto" }, ...hls!.levels.map((l, i) => ({ level: i, label: `${l.height}p` }))]
      renderQuality()
    })
    ...
    hls.on(Hls.Events.ERROR, (_e, data) => {
      ...
      if (recovery < 3 && data.type === Hls.ErrorTypes.NETWORK_ERROR) { recovery++; hls!.startLoad() }
      else if (recovery < 3 && data.type === Hls.ErrorTypes.MEDIA_ERROR) { recovery++; hls!.recoverMediaError() }
      else { hls!.destroy(); showError("Stream unavailable — the demo output may have expired.") }
    })
    ...
    video.addEventListener("error", () => showError("Stream unavailable — the demo output may have expired."))
  } else {
    qualityWrap.hidden = true
  ...
  const onTime = () => {
    if (!seeking && video.duration) seek.value = String((video.currentTime / video.duration) * 1000)
    timeEl.textContent = `${humanDuration(video.currentTime)} / ${humanDuration(video.duration)}`
  }

  playBtn.addEventListener("click", () => void (video.paused ? video.play() : video.pause()))
  ...
  function renderQuality(): void {
    ...
    qualityBtn.textContent = active === -1 ? "Auto" : (levels.find((l) => l.level === active)?.label ?? "Auto")
    qualityMenu.innerHTML = levels
      .map((l) => `<li role="menuitemradio" aria-checked="${l.level === active}" data-level="${l.level}" tabindex="0">${l.label}</li>`)
      .join("")
  }
  ...
  qualityMenu.addEventListener("click", (e) => {
    const li = (e.target as HTMLElement).closest<HTMLLIElement>("li[data-level]")
    if (!li || !hls) return
    hls.currentLevel = Number(li.dataset.level) // -1 = auto; manual pin otherwise
    renderQuality()
    closeMenu()
  })
  const onDocClick = (e: MouseEvent) => { if (!qualityWrap.contains(e.target as Node)) closeMenu() }
