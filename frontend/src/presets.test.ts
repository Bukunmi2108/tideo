...
import { buildPicker, estimateSeconds, LADDER, ESTIMATE_RATIOS } from "./presets"
...
describe("buildPicker", () => {
  ...
  it("returns the full ladder in catalog (highest-first) order", () => {
    ...
    expect(rows.map((r) => r.preset)).toEqual(["1080p", "720p", "480p", "360p"])
  ...
  it("never marks a non-recommended rung available even if heights would allow it", () => {
    ...
    expect(rows.filter((r) => r.available).map((r) => r.preset)).toEqual(["720p"])
  })
...
describe("estimateSeconds", () => {
  it("sums per-preset ratios against the source duration", () => {
    ...
    expect(out).toBeCloseTo(100 * (ESTIMATE_RATIOS["720p"] + ESTIMATE_RATIOS["480p"]))
  })
