...
export function mount(root: HTMLElement): () => void {
  ...
  function bind(): void {
    ...
    root
      ...
      ?.addEventListener("click", () => setState({ tag: "idle" }));
  }
  ...
  function handleFile(file: File): void {
    ...
    if (!ALLOWED_EXTS.includes(ext)) {
      setState({
        ...
        message: `${ext} files are not supported. Please upload a video file.`,
      });
      return;
    }
